...
export function buildSkillsXML(skills: SkillDefinition[]): string {
...
/** 构造工具 XML 列表 */
export function buildToolsXML(tools: ToolDefinition[]): string {
  let xml = '<available_tools>\n';
  for (const tool of tools) {
    const params = tool.parameters.map(p =>
      `      <parameter name="${p.name}" type="${p.type}" required="${p.required}">${escapeXml(p.description)}</parameter>`
    ).join('\n');
    xml += `  <tool>\n`;
...
/** 知识库检索结果的提示词片段 */
export function buildKnowledgeContext(results: { content: string; category: string; score: number }[]): string {
  if (results.length === 0) return '（未从知识库检索到相关内容）';

  const lines = results.map((r, i) =>
    `[知识库#${i + 1}]（${r.category}，置信度${(r.score * 100).toFixed(0)}%）\n${r.content}`
  );
  return `## 知识库检索结果\n${lines.join('\n\n')}`;
}
