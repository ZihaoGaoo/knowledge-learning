...
import { buildKnowledgeContext } from '../agent/prompts';
import { KnowledgeBase } from '../knowledge/KnowledgeBase';

/** MCP 配置 */
export interface MCPConfig {
  maxHistoryMessages: number;   // 最多保留历史消息数
  maxContextEntries: number;    // 知识库最多注入条数
  enableContextCompression: boolean; // 启用上下文压缩
  contextWindowTokens: number;  // 上下文窗口（token 估算）
}
...
export class MCP {
  ...
  private sessionContexts = new Map<string, ConversationContext>();
  ...
  buildContext(sessionId: string, userId: string, userMessage: string): MCPContext {
    ...
    // 4. 用户画像（从元数据恢复）
    const userProfile = ctx.metadata['userProfile'] as MCPContext['userProfile'];

    // 5. 会话变量
    const sessionVars = (ctx.metadata['sessionVars'] as Record<string, unknown>) || {};
  ...
  buildMCPInstructions(mcpCtx: MCPContext): string {
    ...
    if (mcpCtx.knowledgeResults.length > 0) {
      ...
      parts.push(buildKnowledgeContext(formatted));
    }

    // 激活的技能
    if (mcpCtx.activeSkills.length > 0) {
      parts.push(`## 触发的技能\n` +
        mcpCtx.activeSkills.map(s =>
  ...
  /** 匹配技能（基于关键词 + 语义） */
  private matchSkills(message: string): SkillDefinition[] {
